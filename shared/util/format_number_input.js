...
const NumberFormat =  React.createClass({
  ...
  formatInput : function(val){
    const {prefix, thousandSeperator, suffix, mask,format,maxValue} = this.props;
    const maskPattern = format && typeof format == "string" && !!mask;

    if((!val && val !== 0) || !((val+"").match(/\d/g))) return {value :"", formattedValue: (maskPattern ? "" : "")}
    ...
    if(format){
      ...
      if(maxValue && Number(num)> maxValue){
        num = this.formatInput(this.state.value).value;
        formattedValue = num;
      }
  ...
  onChangeHandler : function(e,callback){
    ...
    this.setState({value : formattedValue},()=>{
      ...
      if(callback) callback(e,value,formattedValue);
    });
