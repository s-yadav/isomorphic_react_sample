...
const hoverForMore = function(elm, options) {
  ...
  var targetElm = $(options.target || self);
  ...
  // Attach start event
  targetElm.on(options.startEvent, function(e) {
    startTime = (new Date()).getTime();

    // Get hovered item, and ensure that it contains an overflown item
    var $item = self.filter(":first");
  ...
  // Attach stop event
  if (!options.alwaysOn)
    targetElm.on(options.stopEvent, function(e) {
      var $item = self.filter(":first");
      if (!$item.length) return true;
  ...
  if (options.alwaysOn)
      targetElm.trigger(options.startEvent);

  return self;
};

module.exports = hoverForMore;
